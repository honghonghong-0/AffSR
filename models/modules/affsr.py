"""
models/modules/affsr.py
=======================
AffSR v10 integrated model
...
class AffSR(nn.Module):
    ...
    def forward(
        ...
        score = (r_u_star * e_final_star).sum(dim=-1) \
                - F.softplus(self.lambda_mc) * torch.norm(e_aff_v - a_n, dim=-1)
    ...
    def predict(
        ...
        # AffDrift penalty: score -= softplus(λ) * ‖va_item - a_n‖₂  (modules/affsr.py version)
        if not self.baseline_only:
            va_diff = all_item_va.unsqueeze(0) - a_n.unsqueeze(1)  # (B, N, 2)
            scores = scores - F.softplus(self.lambda_mc) * va_diff.norm(dim=-1)
