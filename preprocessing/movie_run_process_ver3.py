...
top-k ablation 방법 (재추론 없이):
  ...
  --recompute_va 플래그로 k만 바꿔서 VA 값을 재계산 가능:
...
def step3_goemotions(df, output_dir, batch_size=128, device="cpu", top_k=5):
    ...
    if out_path.exists():
        ...
        if cached_top_k == current_top_k_val:
            ...
            # top_k가 다르지만 28차원 raw가 있으면 → 재추론 없이 VA만 재계산
            print(f"[Step3] top_k 불일치 (캐시={cached_top_k}, 요청={current_top_k_val})")
            print(f"         28차원 raw 발견 → GoEmotions 재추론 없이 VA 재계산")
            recompute_va_from_raw(str(out_path), current_top_k_val, str(out_path))
            return pd.read_csv(out_path)
        else:
            print(f"[Step3] top_k 불일치 (캐시={cached_top_k}, 요청={current_top_k_val})")
            print(f"         28차원 raw 없음 → GoEmotions 재추론 필요")
            # 기존 캐시 덮어쓰기 진행
    ...
    # results[idx] = (valence, arousal, dist_28_raw)
    # v,a는 chunk 로드 후 최종 저장 직전에 현재 top_k로 항상 재계산
    # → top_k 변경 재실행 시 chunk v,a와 섞이는 버그 방지
    results = {}  # idx → (v_stale, a_stale, dist28)  ← v,a는 최종 단계에서 재계산
    ...
    for start in tqdm(range(0, len(texts), batch_size), desc="GoEmotions"):
        ...
        # raw 28차원 분포 계산 (neutral 포함, masking 없음 — raw 그대로 저장)
        ...
        for i, df_idx in enumerate(batch_idxs):
            # v, a는 임시값(0.0) — 최종 저장 직전에 재계산하므로 여기선 중요하지 않음
            d = raw_mtx[i].tolist()
    ...
    # ── 최종 CSV 생성: v,a는 현재 top_k로 항상 재계산 ──────────────────────
    # chunk에 저장된 v,a(임시값)를 쓰지 않고,
    # dist28 raw로부터 현재 top_k 기준으로 다시 계산
...
def step4_item_va(review_va_df, output_dir, top_k_val: int = 0):
    ...
    - __meta__ 키로 top_k_used 기록 → top_k 변경 시 stale 캐시 자동 재계산
    """
    ...
    if out_path.exists():
        ...
        if cached_topk == top_k_val:
            print(f"[Step4] 캐시 발견 (top_k={cached_topk}) → 로드: {out_path}")
            return {k: v for k, v in cached.items() if k != "__meta__"}
        print(f"[Step4] top_k 불일치 (캐시={cached_topk}, 요청={top_k_val}) → 재계산")
...
def step5_sequences(df, review_va_df, output_dir, top_k_val: int = 0):
    ...
    if all(p.exists() for p in [train_path, valid_path, test_path]):
        ...
        if cached_topk == top_k_val:
            ...
            return
        print(f"[Step5] top_k 불일치 (캐시={cached_topk}, 요청={top_k_val}) → 재계산")
...
# ─────────────────────────────────────────────────────────────────────────────
# top-k ablation용 VA 재계산 (재추론 불필요)
# CLI: python run_preprocess.py --recompute_va --top_k 3 \
#          --review_va_path .../review_va.csv \
#          --output_path .../review_va_topk3.csv
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    parser.add_argument("--top_k",       type=int, default=5,
                        help="VA 변환 시 사용할 상위 감정 수 (기본값 5). "
                             "0으로 설정하면 masking 없이 전체 사용. "
                             "유효 범위: 0(전체) 또는 2~27.")
    # top-k ablation CLI
    parser.add_argument("--recompute_va",   action="store_true",
                        help="GoEmotions 재추론 없이 저장된 28차원 raw로 VA 재계산")
    parser.add_argument("--review_va_path", default=None,
                        help="--recompute_va 시 입력 review_va.csv 경로")
    parser.add_argument("--output_path",    default=None,
                        help="--recompute_va 시 출력 경로 (미지정 시 입력 파일 덮어쓰기)")
    args = parser.parse_args()

    # ── top_k 유효성 검사 ───────────────────────────────────────────────────
    if args.top_k < 0 or args.top_k == 1 or args.top_k > 27:
        parser.error(
            f"--top_k={args.top_k} 은 유효하지 않습니다. "
            "0(masking 없음) 또는 2~27 사이 정수를 입력하세요."
        )
